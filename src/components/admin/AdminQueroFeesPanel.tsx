...
import { DEFAULT_QUERO_FEES, type QueroFeeSettings } from "@/lib/queroFees";

const sb = supabase as any;

interface RestaurantOption { id: string; name: string }

export function AdminQueroFeesPanel() {
  const [restaurants, setRestaurants] = useState<RestaurantOption[]>([]);
  const [restaurantId, setRestaurantId] = useState<string>("");
  const [settings, setSettings] = useState<QueroFeeSettings>(DEFAULT_QUERO_FEES);
    ...
    (async () => {
      ...
      if (data) {
        setSettings({
          ...
          online_payment_pct: Number(data.online_payment_pct ?? 0),
        });
      } else {
        setSettings(DEFAULT_QUERO_FEES);
      }
      setLoading(false);
    })();
  }, [restaurantId]);
  ...
  const save = async () => {
    if (!restaurantId) return;
    setSaving(true);
    const { error } = await sb.from("quero_fee_settings").upsert({
      restaurant_id: restaurantId,
      ...settings,
    }, { onConflict: "restaurant_id" });
    setSaving(false);
    if (error) return toast.error(error.message);
    toast.success("Taxas da Quero Delivery salvas");
  };

  type FeeKey = "commission" | "online_payment";
  const feeRows: { key: FeeKey; label: string; description: string }[] = [
    { key: "commission", label: "Comissão da plataforma", description: "Comissão percentual cobrada pela Quero sobre cada pedido (padrão 8%)." },
    { key: "online_payment", label: "Taxa de pagamento online", description: "Percentual cobrado quando o pagamento é online (padrão 4,99%)." },
  ];

  const update = (k: FeeKey, field: "enabled" | "pct", v: boolean | number) => {
    setSettings((s) => ({ ...s, [`${k}_${field}`]: v } as QueroFeeSettings));
  };
  ...
  return (
    <div className="space-y-4 max-w-3xl">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><Truck className="w-5 h-5" /> Configurações Quero Delivery</CardTitle>
          <p className="text-sm text-muted-foreground">
            Defina as taxas que a Quero Delivery cobra para que o sistema calcule o valor recebido em cada pedido.
          </p>
        </CardHeader>
