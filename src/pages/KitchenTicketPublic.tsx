...
export default function KitchenTicketPublic() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const [{ data: its }, { data: r }] = await Promise.all([
        ...
        supabase
          .from("restaurants")
          .select("name,logo_url,address_street,address_number,address_neighborhood,address_city,address_state,address_cep,kitchen_print_settings")
  ...
  const ps: PrintSettings = normalizePrintSettings(
    (restaurant as any)?.kitchen_print_settings,
    DEFAULT_KITCHEN_PRINT_SETTINGS,
  );
  ...
  return (
    <>
      <style>{`
        ...
        .ticket { width: 72mm; margin: 0 auto; padding: 8px; font-family: 'Arial Black','Arial',sans-serif; color: #000; font-size: 13px; line-height: 1.45; font-weight: 700; }
        .ticket h1 { font-size: 16px; font-weight: 900; margin: 0; text-align: center; }
        .ticket .muted { color: #000; font-weight: 700; }
        .ticket .center { text-align: center; }
        .ticket .row { display: flex; justify-content: space-between; gap: 8px; }
        .ticket .sep { border-top: 1px solid #000; margin: 6px 0; }
        .ticket .item-name { font-weight: 900; }
